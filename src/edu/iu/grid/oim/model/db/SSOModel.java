...
public class SSOModel extends SSOSmallTableModelBase<SSORecord> {
	...
	public SSORecord getByEmail(String email) throws SQLException
	{
		if(email != null) {
			for(RecordBase it : getCache()) 
			{
				...
				if(rec.email1 != null){
                                System.out.println(rec.email1+"<-- email from inside getbyEmail 1");

				System.out.println(rec.idp+"<-- email from inside getbyEmail IDP");
				}
				if(rec.email.compareTo(email) == 0 ) {
				    
				    System.out.println(rec.email+"<-- email from inside getbyEmail");
				    return rec;
				}

				if(rec.email1 != null){
				    if(rec.email1.compareTo(email) == 0 ) {
					System.out.println(rec.email+"<-- email from inside getbyEmail");
					return rec;
				    }
				}
				...
				if(rec.email2 != null){
                                    if(rec.email2.compareTo(email) == 0 ) {
					System.out.println(rec.email+"<-- email from inside getbyEmail");
                                        return rec;
                                    }
                                }
				...
				if(rec.email3 != null){
                                    if(rec.email3.compareTo(email) == 0 ) {
					System.out.println(rec.email+"<-- email from inside getbyEmail");
                                        return rec;
                                    }
                                }
	...
	public void ifContactExistAdd(String user_email, String dn_string, HttpServletRequest request) throws SQLException {
	    ...
	    String OIDC_CLAIM_access_token = (String)request.getHeader("OIDC_CLAIM_access_token");
	    String OIDC_CLAIM_email = (String)request.getHeader("OIDC_CLAIM_email");
	    String OIDC_CLAIM_family_name = (String)request.getHeader("OIDC_CLAIM_family_name");
	    String OIDC_CLAIM_given_name = (String)request.getHeader("OIDC_CLAIM_given_name"); 
	    String OIDC_CLAIM_idp = (String)request.getHeader("OIDC_CLAIM_idp");
	    String OIDC_CLAIM_idp_name = (String)request.getHeader("OIDC_CLAIM_idp_name");
	    ResultSet index_rs = null;
	    Statement index_stmt = conn.createStatement();

	    System.out.println("*************************** this is user DN "+ sso_user_dn_tmp);
	    // check if this email is already there is not create a contact
	    ...
	    String contact_insert_sql = "INSERT INTO contact (name, primary_email, profile,use_twiki,twiki_id,count_usercert_year,count_hostcert_day,count_hostcert_year ) values ('"+OIDC_CLAIM_given_name+" "+OIDC_CLAIM_family_name+"', '"+OIDC_CLAIM_email+"','none',1,'no1',0,0,0)";

	    PreparedStatement statement = conn.prepareStatement(contact_insert_sql, Statement.RETURN_GENERATED_KEYS);
	    //  Statement stmt = conn.createStatement();

	    String sso_insert_sql = "INSERT INTO contact_authorization_type (given_name,family_name, email,contact_id,access_token,idp,idp_name,created) values ('"+OIDC_CLAIM_given_name+"' ,'"+OIDC_CLAIM_family_name+"','"+OIDC_CLAIM_email+"'," + contact_id + ",'"+OIDC_CLAIM_access_token+"','"+OIDC_CLAIM_idp+"' ,'"+OIDC_CLAIM_idp_name+"',now())";
            PreparedStatement insert_sso_stmt = sso_conn.prepareStatement(sso_insert_sql, Statement.RETURN_GENERATED_KEYS);
	   
	  	    // while (rs_sso1.next()) {
	    // Integer contact_authorization_type_id1  = rs_sso1.getInt(1);
	    // System.out.println(" ###########   inside first loop last inserted contact_authorization_type_id "+ contact_authorization_type_id1);
	    // Statement insert_sso_stmt = sso_conn.createStatement();
	    //}
		...
		System.out.println("condition if sso does not exist but contact info exists:  " +sso_insert_sql);
		...
		insert_sso_stmt.executeUpdate();
		    ...
		    //move actions from dn_authorization_type
		    
		    //Statement insert_dn_sso_stmt = sso_conn.createStatement();
		    
		    //Integer contact_authorization_type_id = insert_dn_sso_stmt.executeUpdate(sso_dn_insert_sql);
		 
		    
		    String select_dn_contact ="select * from dn inner join contact on contact.id=dn.contact_id where contact_id="+ contact_id +" order by dn.id desc limit 1";
			...
			s_email_seq = " , email2='" + s_email + "'" ;
                    ...
                    String update_dn_contact_with_contact ="update contact_authorization_type  set email1='"+p_email+"' " + s_email_seq + "  where contact_id="+contact_id+"";
	    ...
	    }else if(sso_exist==0 && contact_exist==0 && dn_exist==1){
		...
		ResultSet dn_contact_rs = dn_contact_stmt.executeQuery(select_dn_contact);
		System.out.println("SSO NO --- Contact NO --- DN YES");
		if(dn_contact_rs.next()){

		    String email_seq;
                    String sso_dn_id = dn_contact_rs.getString("id");
		    ...
		    String update_dn_contact ="update contact_authorization_type  set "+email_seq+"='"+user_email+"' where contact_id="+dn_contact_id+"";
		    System.out.println(update_dn_contact);
		    Statement update_dn_contact_stmt = sso_conn.createStatement();
		    update_dn_contact_stmt.executeUpdate(update_dn_contact);
		    update_dn_contact_stmt.close();
		    //load update actions
		    if (index_stmt.execute("SELECT * FROM dn_authorization_type WHERE dn_id = " + sso_dn_id)) {
                        index_rs = index_stmt.getResultSet();
                        while (index_rs.next()) {
                            authorization_type_id =index_rs.getInt("authorization_type_id");
		    ...
		    String sso_dn_insert_sql = "INSERT INTO contact_authorization_type (given_name,family_name, email,contact_id,access_token,idp,idp_name,email1) values ('"+OIDC_CLAIM_given_name+"' ,'"+OIDC_CLAIM_family_name+"','"+OIDC_CLAIM_email+"'," + dn_contact_id + ",'"+OIDC_CLAIM_access_token+"','"+OIDC_CLAIM_idp+"' ,'"+OIDC_CLAIM_idp_name+"','"+p_email0+"')";

		    //"INSERT INTO contact_authorization_type (given_name,family_name, email,contact_id) values ('Marina','Krenz', 'mvkrenz@iu.edu'," + dn_contact_id + ")";
		    Statement insert_dn_sso_stmt = sso_conn.createStatement();
		    contact_authorization_type_id = insert_dn_sso_stmt.executeUpdate(sso_dn_insert_sql);
	    ...
	    if(sso_exist==0 && contact_exist==0 && dn_exist==0){
		...
		while (rs_dn.next()) {
                    ...
                    contact_id = rs_dn.getInt(1);

		    sso_insert_sql = "INSERT INTO contact_authorization_type (given_name,family_name, email,contact_id,access_token,idp,idp_name,created) values ('"+OIDC_CLAIM_given_name+"','"+OIDC_CLAIM_family_name+"','"+OIDC_CLAIM_email+"'," + contact_id + ",'"+OIDC_CLAIM_access_token+"','"+OIDC_CLAIM_idp+"' ,'"+OIDC_CLAIM_idp_name+"',now())";
 
		    PreparedStatement insert_sso_stmt1 = sso_conn.prepareStatement(sso_insert_sql, Statement.RETURN_GENERATED_KEYS);

		    System.out.println("contact _id "+contact_id+"  :  " + sso_insert_sql);
		    insert_sso_stmt1.executeUpdate(sso_insert_sql);
		    }
	    }
